...
| **Observabilidade** | Logging estruturado (`structlog`) em todos os módulos; relatório de qualidade de dados gerado a cada execução; métricas Prometheus Fase 0+1 (Sprint 22, ADR-051): exporter dedicado (`pipeline_last_run_*`, `dq_*`, `gold_*`) + `GET /metrics` na API (`http_*`, `gold_*`), scrape 15s; Grafana Fase 2 (Sprint 23, ADR-053): dashboard SLOs provisionado como código + 8 regras de alerta versionadas (`infra/observability/alerts.yml`), UI em `127.0.0.1:3000` sem Nginx; framework 5 níveis (Sprint 26, ADR-056): status granular (`status_detalhado`) + duração por task (`pipeline_task_*`), MTTR/MTBF (30 runs), Cobertura (1/dia, 7d), Health Index 40/30/20/10 com classes, heatmap/timeline + painéis novos, Alertmanager (`127.0.0.1:9093`, destino default `blackhole`, Telegram ativo p/ `critical` via `bot_token_file` — `chat_id`/token reais pendentes do operador, ADR-057; Slack como template, soma não substitui), score DQ agregado PASS/WARN/FAIL — 14 regras no total |
