...
| **Observabilidade** | Logging estruturado (`structlog`) em todos os módulos; relatório de qualidade de dados gerado a cada execução; métricas Prometheus Fase 0+1 (Sprint 22, ADR-051): exporter dedicado (`pipeline_last_run_*`, `dq_*`, `gold_*`) + `GET /metrics` na API (`http_*`, `gold_*`), scrape 15s; Grafana Fase 2 (Sprint 23, ADR-053): dashboard SLOs provisionado como código + 8 regras de alerta versionadas (`infra/observability/alerts.yml`), UI em `127.0.0.1:3000` sem Nginx |
